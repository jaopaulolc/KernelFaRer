//===- KernelMatcher.cpp - Kernel Idiom Recognition Pass ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This pass implements an idiom recognizer that identifies GEMM and SYR2K in
// normalized loops. This pass is used by the KernelReplacerPass to identify
// idiomatic instances that can be replaced by calls to CBLAS or EIGEN.
//
//===----------------------------------------------------------------------===//
//
// TODO List:
...
#define DEBUG_TYPE "kernel-matcher"
...
static bool matchGEMM(Instruction &SeedInst, Value *&IVarI, Value *&IVarJ,
                      ...
                      Value *&Beta, bool &CIsPartOfReduction) {
  ...
  if (match(SeedInstAsValue, Matcher) && BasePtrToA != BasePtrToC &&
    ...
    if (Alpha == nullptr)
      Alpha = Alpha1;
    CIsPartOfReduction = MatchedGEP != nullptr || Beta != nullptr;
    IsGEMM = true;
  }
  return IsGEMM;
}
...
static void collectOtherKernelStoresToC(
    ...
    SmallSetVector<const Value *, 2> &Stores, bool &CIsPartOfReduction) {
      ...
      for (auto Inst = BB->begin(); Inst != BB->end(); Inst++) {
        ...
        if (matchSYR2K(*Inst, IVarI, IVarJ, IVarK, BasePtrToA, BasePtrToB,
          ...
          // Matrix-Multiply pattern
                             ...
                             CLayout, LI, Alpha, Beta, CIsPartOfReduction) &&
        ...
        collectOtherKernelStoresToC(BasePtrToC, MatchedLDC, IVarI, IVarJ, Alpha,
                                    Beta, OuterLoop, *Inst, DT, Stores, CIsPartOfReduction);
