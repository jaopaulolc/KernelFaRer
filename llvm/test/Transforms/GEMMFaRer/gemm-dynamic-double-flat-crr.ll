...
define void @_Z14dynamicSizeGEMMiiiPKdiS0_iPdidd(i32 %m, i32 %n, i32 %k, ptr %A, i32 %lda, ptr %B, i32 %ldb, ptr %C, i32 %ldc, double %alpha, double %beta) {
; CHECK-LABEL: @_Z14dynamicSizeGEMMiiiPKdiS0_iPdidd(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    call void @cblas_dgemm(i32 101, i32 112, i32 111, i32 [[M:%.*]], i32 [[N:%.*]], i32 [[K:%.*]], double [[ALPHA:%.*]], ptr [[A:%.*]], i32 [[LDA:%.*]], ptr [[B:%.*]], i32 [[LDB:%.*]], double [[BETA:%.*]], ptr [[C:%.*]], i32 [[LDC:%.*]])
; CHECK-NEXT:    ret void
;
entry:
...
for.cond.cleanup7:                                ; preds = %for.body8, %for.cond5.preheader
  ...
  %arrayidx19 = getelementptr inbounds double, ptr %C, i64 %4
  %5 = load double, ptr %arrayidx19, align 8
